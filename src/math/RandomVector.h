...
namespace arx {
	...
	inline Vec3f randomOffsetXZ(float range) {
		return Vec3f(g_rand.getf(-range, range), 0.f, g_rand.getf(-range, range));
	}
	...
	inline Vec3f randomVec(float min = 0.f, float max = 1.f) {
		float range = max - min;
		return Vec3f(g_rand.getf() * range + min, g_rand.getf() * range + min, g_rand.getf() * range + min);
	}
	
	inline Vec3f randomVec3f() {
		return Vec3f(g_rand.getf(), g_rand.getf(), g_rand.getf());
	}
	
	inline Vec2f linearRand(Vec2f const & min, Vec2f const & max) {
		return Vec2f(g_rand.getf(min.x, max.x), g_rand.getf(min.y, max.y));
	}
	
	inline Vec3f linearRand(Vec3f const & min, Vec3f const & max) {
		return Vec3f(g_rand.getf(min.x, max.x), g_rand.getf(min.y, max.y), g_rand.getf(min.z, max.z));
	}
