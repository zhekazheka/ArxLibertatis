	...
	if (inter.iobj[0])
	{
		...
		if (!BLOCK_PLAYER_CONTROLS)
			...
			if (ause0->flags & EA_FORCEPLAY)
			{
				...
				{
					ause0->flags &= ~EA_STATICANIM;
					io->pos.x = pos.x = moveto.x = pos.x + io->move.x;
					io->pos.y = pos.y = moveto.y = pos.y + io->move.y;
					io->pos.z = pos.z = moveto.z = pos.z + io->move.z;
				...
				&&  (!(Current_Movement & PLAYER_CROUCH)))
			{
				if ((Current_Movement & PLAYER_LEAN_LEFT)
					&&  (Current_Movement & PLAYER_LEAN_RIGHT))
				{
				}
				else
				{
