...
void createFireParticles(Vec3f & pos, int perPos, int delay) {
	for(long nn = 0 ; nn < perPos; nn++) {
		...
		pd->ov = pos;
		pd->move = Vec3f(2.f, 2.f, 2.f) - Vec3f(4.f, 22.f, 4.f) * g_rand.randomVec3f();
		pd->siz = 7.f;
...
void createObjFireParticles(const EERIE_3DOBJ * obj, int particlePositions, int perPos, int delay) {
	...
	for(int i = 0; i < particlePositions; i++) {
		...
		while(notok-- > 0) {
			it = g_rand.getIterator(obj->facelist);
...
void ARX_PARTICLES_Spawn_Lava_Burn(Vec3f pos, Entity * io) {
	...
	if(io && io->obj && !io->obj->facelist.empty()) {
		...
		long notok = 10;
		while(notok-- > 0) {
			num = g_rand.getu(0, io->obj->facelist.size() - 1);
	...
	pd->ov = pos;
	pd->move = g_rand.randomVec3f() * Vec3f(2.f, -12.f, 2.f) - Vec3f(4.f, 15.f, 4.f);
	pd->tolive = 800;
	pd->tc = smokeparticle;
	pd->siz = 15.f;
	pd->scale = g_rand.randomVec(15.f, 20.f);
...
static void ARX_PARTICLES_Spawn_Rogue_Blood(const Vec3f & pos, float dmgs, Color col) {
	...
	pd->tolive = 1600;
	pd->move = g_rand.randomVec3f() * Vec3f(60.f, -10.f, 60.f) - Vec3f(30.f, 15.f, 30.f);
	pd->rgb = col.to<float>();
	long num = g_rand.get(0, 5);
	pd->tc = bloodsplat[num];
	pd->m_rotation = g_rand.getf(-0.05f, 0.05f);
...
void ARX_PARTICLES_Spawn_Blood(const Vec3f & pos, float dmgs, EntityHandle source) {
	...
	for(unsigned int k = 0; k < spawn_nb; k++) {
		...
		totdelay += 45 + g_rand.getu(0, 150 - spawn_nb);
		pd->delay = totdelay;
...
void AddRandomSmoke(Entity * io, long amount) {
	...
	for(long i = 0; i < amount; i++) {
		...
		long vertex = g_rand.get(0, io->obj->vertexlist.size() - 1);
		pd->ov = io->obj->vertexlist3[vertex].v + g_rand.randomVec(-5.f, 5.f);
		pd->siz = g_rand.getf(0.f, 8.f);
		if(pd->siz < 4.f) {
			pd->siz = 4.f;
		}
		pd->scale = Vec3f(10.f);
		pd->m_flags = ROTATING | FADE_IN_AND_OUT;
		pd->tolive = g_rand.getu(900, 1300);
		pd->move = g_rand.linearRand(Vec3f(-0.25f, -0.7f, -0.25f), Vec3f(0.25f, 0.3f, 0.25f));
...
void ARX_PARTICLES_Add_Smoke(const Vec3f & pos, long flags, long amount, const Color3f & rgb) {
	...
	Vec3f mod = (flags & 1) ? g_rand.randomVec(-50.f, 50.f) : Vec3f_ZERO;
	...
	while(amount--) {
		...
		pd->ov = pos + mod;
		if(flags & 2) {
			pd->siz = g_rand.getf(15.f, 35.f);
			pd->scale = g_rand.randomVec(40.f, 55.f);
		} else {
			pd->siz = g_rand.getf(5.f, 13.f);
			pd->scale = g_rand.randomVec(10.f, 15.f);
		}
		pd->m_flags = ROTATING | FADE_IN_AND_OUT;
		pd->tolive = g_rand.getu(1100, 1500);
		pd->delay = amount * 120 + g_rand.getu(0, 100);
		pd->move = g_rand.linearRand(Vec3f(-0.25f, -0.7f, -0.25f), Vec3f(0.25f, 0.3f, 0.25f));
		pd->rgb = rgb;
		pd->tc = smokeparticle;
		pd->m_rotation = 0.01f;
...
void MagFX(const Vec3f & pos, float size) {
	...
	pd->ov = pos + Vec3f(g_rand.getf(0.f, 6.f) - g_rand.getf(0.f, 12.f), g_rand.getf(0.f, 6.f) - g_rand.getf(0.f, 12.f), 0.f);
	pd->move = Vec3f(g_rand.getf(-6.f, 6.f), g_rand.getf(-8.f, 8.f), 0.f);
	pd->scale = Vec3f(4.4f, 4.4f, 1.f);
	pd->tolive = g_rand.getu(1500, 2400);
	pd->tc = healing;
...
void ARX_PARTICLES_Spawn_Splat(const Vec3f & pos, float dmgs, Color col) {
	...
	for(long kk = 0; kk < 20; kk++) {
		...
		pd->ov = pos;
		pd->move = g_rand.randomVec(-11.5f, 11.5f);
		pd->tolive = tolive;
		pd->tc = blood_splat;
...
void ARX_PARTICLES_SpawnWaterSplash(const Vec3f & _ePos) {
	...
	long nbParticles = g_rand.get(15, 35);
	for(long kk=0; kk < nbParticles; kk++) {
		...
		pd->ov = _ePos + Vec3f(30.f, -20.f, 30.f) * g_rand.randomVec3f();
		pd->move = g_rand.linearRand(Vec3f(-6.5f, -11.5f, -6.5f), Vec3f(6.5f, 0.f, 6.5f));
		pd->tolive = g_rand.getu(1000, 1300);
...
void SpawnFireballTail(const Vec3f & poss, const Vec3f & vecto, float level, long flags) {
	...
	for(long nn = 0; nn < 2; nn++) {
		...
		pd->m_rotation = g_rand.getf(0.f, 0.02f);
		pd->move = Vec3f(0.f, g_rand.getf(-3.f, 0.f), 0.f);
		pd->tc = explo[0];
		pd->rgb = Color3f::gray(.7f);
		pd->siz = (level + g_rand.getf()) * 2.f;
		
		if(flags & 1) {
			pd->tolive = g_rand.getu(400, 500);
			pd->siz *= 0.7f;
			pd->scale = Vec3f(level * 1.4f);
		} else {
			pd->scale = Vec3f(level * 2.f);
			pd->tolive = g_rand.getu(800, 900);
		}
...
void LaunchFireballBoom(const Vec3f & poss, float level, Vec3f * direction, Color3f * rgb) {
	...
	pd->ov = poss;
	pd->move = (direction) ? *direction : Vec3f(0.f, g_rand.getf(-5.f, 0.f), 0.f);
	pd->tolive = g_rand.getu(1600, 2200);
	pd->tc = explo[0];
	pd->siz = level * 3.f + g_rand.getf(0.f, 2.f);
...
void spawnFireHitParticle(const Vec3f & poss, long type) {
	...
	if(pd) {
		...
		pd->ov = poss;
		pd->move = Vec3f(3.f, 4.f, 3.f) - Vec3f(6.f, 12.f, 6.f) * g_rand.randomVec3f();
		pd->tolive = g_rand.getu(600, 700);
		pd->tc = tc1;
		pd->siz = g_rand.getf(100.f, 110.f) * ((type == 1) ? 2.f : 1.f);
		pd->zdec = true;
		if(type == 1) {
		...
		if(pd) {
			...
			pd->tolive = g_rand.getu(600, 700);
			pd->tc = tc1;
			pd->siz = g_rand.getf(40.f, 70.f) * ((type == 1) ? 2.f : 1.f);
			pd->zdec = true;
			if(type == 1) {
				pd->rgb = Color3f(.4f, .4f, 1.f);
			}
...
void spawn2DFireParticle(const Vec2f & pos, float scale) {
	...
	pd->move = Vec3f(g_rand.getf(-1.5f, 1.5f), g_rand.getf(-6.f, -5.f), 0.f) * scale;
	...
	for(size_t i = 0; i < MAX_PARTICLES && pcc > 0; i++) {
		...
		if(part->delay > 0) {
			...
			if((part->m_flags & DELAY_FOLLOW_SOURCE) && target) {
				...
				part->move = vector * Vec3f(18.f, 5.f, 18.f) + g_rand.randomVec(-0.5f, 0.5f);
		...
		if(framediff <= 0) {
			if((part->m_flags & FIRE_TO_SMOKE) && g_rand.getf() > 0.7f) {
		...
		if(!part->is2D) {
			...
			if(part->m_flags & SPLAT_WATER) {
				float siz = part->siz + part->scale.x * fd;
				sp.radius = siz * g_rand.getf(10.f, 30.f);
				if(CheckAnythingInSphere(sp, EntityHandle_Player, CAS_NO_NPC_COL)) {
					if(g_rand.getf() < 0.9f) {
...
void TreatBackgroundActions() {
	...
	for(size_t i = 0; i < g_staticLightsMax; i++) {
		...
		if(gl->sample == audio::INVALID_ID) {
			...
			ARX_SOUND_PlaySFX(gl->sample, &gl->pos, g_rand.getf(0.95f, 1.05f), ARX_SOUND_PLAY_LOOPED);
		} else {
		...
		for(long n = 0; n < count; n++) {
			...
			if(g_rand.getf() < gl->ex_frequency) {
				PARTICLE_DEF * pd = createParticle();
				if(pd) {
					float t = g_rand.getf() * glm::pi<float>();
					Vec3f s = Vec3f(std::sin(t), std::sin(t), std::cos(t)) * g_rand.randomVec();
					pd->ov = gl->pos + s * gl->ex_radius;
					pd->move = Vec3f(2.f, 2.f, 2.f) - Vec3f(4.f, 22.f, 4.f) * g_rand.randomVec3f();
					pd->move *= gl->ex_speed;
					pd->siz = 7.f * gl->ex_size;
					pd->tolive = 500 + g_rand.getu(0, 1000 * gl->ex_speed);
					if((gl->extras & EXTRAS_SPAWNFIRE) && (gl->extras & EXTRAS_SPAWNSMOKE)) {
						pd->m_flags = FIRE_TO_SMOKE;
					}
					pd->tc = (gl->extras & EXTRAS_SPAWNFIRE) ? fire2 : smokeparticle;
					pd->m_flags |= ROTATING;
					pd->m_rotation = 0.1f - g_rand.getf(0.f, 0.2f) * gl->ex_speed;
					pd->scale = Vec3f(-8.f);
					pd->rgb = (gl->extras & EXTRAS_COLORLEGACY) ? gl->rgb : Color3f::white;
				}
			}
			
			if(!(gl->extras & EXTRAS_SPAWNFIRE) || g_rand.getf() <= 0.95f) {
				continue;
			}
			...
			if(g_rand.getf() < gl->ex_frequency) {
				...
				if(pd) {
					...
					pd->move = Vec3f(vect.x * d, g_rand.getf(-18.f, -10.f), vect.z * d) * gl->ex_speed;
					pd->siz = 4.f * gl->ex_size * 0.3f;
					pd->tolive = 1200 + g_rand.getu(0, 500 * gl->ex_speed);
					pd->tc = fire2;
