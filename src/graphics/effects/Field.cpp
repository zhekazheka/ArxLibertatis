...
void CCreateField::RenderQuad(const Vec3f & p1, const Vec3f & p2, const Vec3f & p3, const Vec3f & p4, int rec, Vec3f norm, RenderMaterial & mat)
{
	if(rec < 3) {
		...
		q.v[1].color = q.v[2].color = Color3f(falpha * .3f + g_rand.getf(0.f, .025f), 0.f, falpha * .5f + g_rand.getf(0.f, .025f)).toRGB();
		q.v[0].color = q.v[3].color = Color3f(falpha * .3f + g_rand.getf(0.f, .025f), 0.f, falpha * .5f + g_rand.getf(0.f, .025f)).toRGB();
...
void CCreateField::Render()
{
	...
	falpha = glm::sin(glm::radians(fglow)) + g_rand.getf(0.f, 0.2f);
